    ...
    PriceBreakdown calculate(PricingContext pricingContext) {
        ...
        Map<UUID, Money> pricingLines = pricingContext.productsWithQuantities().entrySet().stream()
                .collect(Collectors.toMap(
                        ...
                        entry -> {
                            Money basePrice = pricingContext.exchangeRates()
                                    .convert(entry.getKey().price(), pricingContext.targetCurrency())
                                    .multiply(BigDecimal.valueOf(entry.getValue()));

                            Money totalDiscountAmount = pricingContext.discounts().stream()
                                    .map(discount -> discount.calculateDiscount(basePrice))
                                    .reduce(Money.zero(pricingContext.targetCurrency()), Money::add);

                            return basePrice.subtract(totalDiscountAmount);
                        }
                ));

        Money total = pricingLines.values().stream()
                .reduce(Money.zero(pricingContext.targetCurrency()), Money::add);
